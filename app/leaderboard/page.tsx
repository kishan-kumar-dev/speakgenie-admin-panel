    ...
    <div className="space-y-6">
      <div className="card p-6">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          {(data.leaderboard as any).slice(0,3).map((s:any,idx:number)=> (
            <div key={s.id} className={"card p-5 text-center " + (idx===0 ? "ring-2 ring-yellow-300" : "")}>
      ...
      <div className="card p-6">
        ...
        <div className="space-y-3">
          {(data.leaderboard as any).map((s:any, idx:number)=> (
            <LeaderboardCard key={s.id} rank={idx+1} name={s.name} klass={s.class} points={s.points} accuracy={s.accuracy} />
          ))}
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-4 gap-4">
        <div className="card p-4 text-center">
          ...
          <div className="text-xl font-semibold">{(data.leaderboard as any)[0].name}</div>
          <div className="text-slate-500">{(data.leaderboard as any)[0].points} points</div>
        </div>
        <div className="card p-4 text-center">
